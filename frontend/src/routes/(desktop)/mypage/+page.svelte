<script lang="ts">
	...
	const latestReturn = $derived(portfolio ? portfolio.totalProfitLossPercent : performanceData[performanceData.length - 1]);
	...
	onMount(async () => {
		...
		try {
			const [enrichedPortfolio, tradesRes, stockMap] = await Promise.all([
				...
				getTradeHistory(userId).catch(() => null),
				getStockLookupMap()
			]);
			...
			if (tradesRes?.success && tradesRes.data) {
				transactions = mapTradesToTransactions(tradesRes.data, userId, stockMap);
			}
		} catch (e) {
...
<div class="mypage">
	...
	{#if !$currentUser}
		<div class="text-center p-lg">
			...
			<a href="/login" class="btn btn-primary mt-md">로그인</a>
		</div>
	{:else if loading}
		<div class="text-center p-lg">
	...
	{:else}
		...
		<div class="mypage-profile">
			...
			<div class="mypage-profile-info">
				<div class="mypage-profile-name">{$currentUser.username}</div>
				<div class="mypage-profile-meta">
					<span>{$currentUser.email}</span>
				</div>
			</div>
			<div class="mypage-profile-stat">
				<div class="mypage-profile-stat-value">{portfolio ? portfolio.holdings.length : 0}</div>
			...
			<div class="mypage-profile-stat">
				...
				{:else}
					<div class="mypage-profile-stat-value">0.00%</div>
				{/if}
		...
		<div class="stat-cards-grid mb-lg">
			<div class="card card-hover animate-slide-up" style="animation-delay: 0ms">
				<div class="card-stat">
					...
					<div class="card-stat-value">₩{formatPrice(portfolio?.totalAssetValue ?? 0)}</div>
				</div>
			</div>
			<div class="card card-hover animate-slide-up" style="animation-delay: 50ms">
				<div class="card-stat">
					...
					<div class="card-stat-value">₩{formatPrice(portfolio?.cashBalance ?? 0)}</div>
				</div>
			</div>
			<div class="card card-hover animate-slide-up" style="animation-delay: 100ms">
				<div class="card-stat">
					...
					<div class="card-stat-value">₩{formatPrice(portfolio?.totalStockValue ?? 0)}</div>
				</div>
			</div>
					...
					<div class="performance-chart-container">
						<div class="performance-chart-header">
							<div>
								...
								<div class="text-sm text-secondary">총 수익률</div>
							</div>
							{#if latestReturn >= 0}
